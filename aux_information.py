...
def generar_grafico_compatibilidad(compatibilidad):
    ...
    sns.barplot(x=compatibilidad.index, y=compatibilidad.values, ax=ax, color='pink', edgecolor=None)
...
def generar_tabla_compatibilidad(resultado):
    ...
    fig_table = go.Figure(data=[go.Table(
        ...
        header=dict(values=list(resultado_0_with_index.columns),
                    fill_color='#ED3163',
                    align='left',
                    font=dict(color='white')),  # Cambiar color de las letras del encabezado
        cells=dict(values=[resultado_0_with_index[col] for col in resultado_0_with_index.columns],
                fill_color='lavender',
                align='left',
                font=dict(color='black'))  # Cambiar color de las letras de las celdas
        )
    ])
